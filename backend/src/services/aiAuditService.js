...
const AUDIT_SCHEMA = {
  ...
  properties: {
    ...
    suggested_adjusted_amount: {
      type: ['number', 'null'],
      description: 'Only when the receipt proves MATERIALLY less than the claim — the rupee amount actually evidenced. A gap under about ₹10 is rounding or an OCR misread of the paise, not a discrepancy: leave this null and approve. Setting it blocks automatic approval and pushes work onto a person, so use it for real gaps only.',
    },
    reconciliation_note: {
      type: ['string', 'null'],
...
4. **Dates. Read the year off the receipt before you reason about it.** Ride and UPI apps very often print only a day and month — "12 Aug", "5 September" — with no year at all. If no year is printed, it is the most recent occurrence of that date, almost always this year. Do NOT assume an old year and then treat your own assumption as a discrepancy; that turns an ordinary two-week-old receipt into a two-year-old one. Only call a date old if the year is actually printed on the receipt.

   Employees have 7 days to file and routinely submit a week of receipts at once, so a receipt older than the submission is normal. **Spending out of pocket before the advance arrived and claiming it afterwards is explicitly permitted here** — a receipt predating the payout is fine on its own and is NOT a reason to reject or to raise a fraud signal. Mention it in your reasoning if you like, then move on. The only date facts worth escalating are a receipt dated *after* submission, or one from a period so distant it cannot plausibly relate to this advance — and even then the verdict is needs_human, so a person can ask.
5. **Small amount differences are immaterial — approve them.** Anything within about ₹10 either way is rounding, a convenience fee, or an OCR misread of the paise. A receipt for ₹819.05 against a claim of ₹820 is a match: approve it, do not suggest an adjustment, and never ask the employee to resubmit over it. Making someone re-file for 95 paise is exactly the busywork this system exists to remove.
...
12. **Write employee_fix_hint ONLY when re-sending a file would actually fix it** — an unreadable screenshot, a bill instead of a payment confirmation, the wrong screenshot attached. The only thing they can do is upload a different image, so never ask them to "clarify", "explain" or "confirm" something: that is a question for finance, and the verdict there is needs_human with a hint of null. If you believe they attached the wrong receipt, say exactly that and ask for the right one. Write it in **Hinglish** (simple Hindi in Roman script mixed with everyday English words), because that is how the site staff who read it actually communicate. One short sentence telling them exactly what to send instead. Leave it null when there is nothing for them to fix — everything else you write stays in English for the finance team.
...
function buildContextBlock(ctx) {
  ...
  sections.push(
    [
      ...
      `Today's date: ${new Date().toISOString().slice(0, 10)} — use it to resolve any receipt that prints a day and month but no year.`,
