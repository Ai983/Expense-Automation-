...
function ImprestRemindersPanel() {
  ...
  // Match on employee name, either site, or the imprest ref — with 285 reminders
  // the list is unusable without it.
  ...
  const overdueCount = reminders.filter(r => r.status === 'expired' || new Date(r.deadline) <= new Date()).length;
    ...
    <div className="mb-5">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-amber-800">⏰ Imprest Expense Reminders</span>
          <span className="text-xs bg-amber-200 text-amber-800 font-bold px-2 py-0.5 rounded-full">{reminders.length}</span>
          {overdueCount > 0 && (
            <span className="text-xs bg-red-100 text-red-700 font-bold px-2 py-0.5 rounded-full">⚠ {overdueCount} overdue</span>
          )}
        </div>
        <span className="text-amber-600 text-sm">{collapsed ? '▼ Show' : '▲ Hide'}</span>
      </button>

      {!collapsed && (
        <div className="bg-white rounded-b-xl border border-t-0 border-amber-200 overflow-hidden">
          <div className="flex items-center gap-3 px-4 py-2.5 bg-amber-50/60 border-b border-amber-100">
            <div className="relative flex-1 max-w-sm">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-1.5 text-sm border border-amber-200 rounded-lg bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-300 focus:border-amber-300"
              />
              {search && (
                <button
                  onClick={() => setSearch('')}
          ...
          {/* Height-capped scrollable table — max 6 rows visible */}
          <div className="overflow-auto" style={{ maxHeight: '260px' }}>
            <table className="w-full text-sm">
              <thead className="bg-amber-50 border-b border-amber-100 sticky top-0 z-10">
                <tr>
                  ...
                  return (
                    <tr key={r.id} className={isExpired ? 'bg-red-50' : 'hover:bg-amber-50 transition-colors'}>
                      ...
                      <td className="px-4 py-2">
                        ...
                        {isExpired && <div className="text-xs text-red-600 font-semibold">Overdue</div>}
                      </td>
                      <td className="px-4 py-2">
                        {isExpired && r.employee?.imprest_blocked ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-red-100 text-red-700">Blocked</span>
                        ) : isExpired ? (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-orange-100 text-orange-700">Overdue</span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-yellow-100 text-yellow-800">Pending</span>
                        )}
                      </td>
                      <td className="px-4 py-2">
                        {isExpired && r.employee?.imprest_blocked && (
